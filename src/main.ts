  ...
  if (fileSuffix === "ts") {
    ...
    if (multipleFiles) {
      controllerList.forEach(async ({ description, controllerName, funcList }) => {
        ...
        const template_Api = await ejsRender(ejs_template, renderOptionsApi);
        outputFile(outputDir, `${serviceName}/${controllerName}.${fileSuffix}`, template_Api, prettierOptions);
      });
    } else {
      ...
      const template_Api = await ejsRender(ejs_template, renderOptionsApi);
      outputFile(outputDir, `${serviceName}/api.${fileSuffix}`, template_Api, prettierOptions);
    }
  }
